...
.onAttach <- function(libname, pkgname) {
    if (interactive()) {
        ...
        installed.version <- utils::packageDescription("packageSkeleton")[['Version']]
        cran.version <- tryCatch(
            green("v", pkgsearch::cran_package("packageSkeleton")[['Version']], sep=""),
            error = function(e) red("Not Available"),
            warning = function(w) red("Not Available"))
        dev.version <- get_dev_version("packageSkeleton")

        # Define a friendly startup message
		message_text <- paste0(
		    magenta(bold("\uD83C\uDF89 packageSkeleton v", installed.version, sep="")), " - ", toOrdinal::toOrdinalDate("2025-7-9"), "\n",
			strrep("\u2501", 40), "\n",
